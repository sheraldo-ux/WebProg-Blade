      ...
      <div class="bg-white p-6 rounded-lg shadow">
        ...
        @if($users->count() > 0)
          ...
          <div class="overflow-x-auto mt-6 w-full">
            <form method="GET" action="{{ route('profile.admin.index') }}" class="mb-4 flex justify-between items-center">
              <div>
            ...
            <table class="w-full border-collapse">
              <thead>
                <tr>
                  <th class="text-left text-neutral-900 px-3 py-3 bg-neutral-100 rounded-l-lg border border-gray-300">ID</th>
                  ...
                  <th class="text-left text-neutral-900 px-3 py-3 bg-neutral-100 rounded-r-lg border border-gray-300">Actions</th>
                </tr>
              </thead>
                ...
                @foreach ($users as $user)
                  <tr class="border-b border-gray-200">
                    ...
                    <td class="px-3 py-3 border border-gray-200 flex space-x-2">
                      <form action="{{ route('profile.admin.delete_user', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this user?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-red-600 text-white px-2 py-1 rounded-md hover:bg-red-700">Delete</button>
                      </form>
                      <a href="{{ route('profile.admin.view_update', $user->id) }}" class="bg-blue-600 text-white px-2 py-1 rounded-md hover:bg-blue-700">Update</a>
                    </td>
                  </tr>
                @endforeach
              </tbody>
              ...
              @if ($users instanceof \Illuminate\Pagination\LengthAwarePaginator)
                ...
                <div>
                  <form method="GET" action="{{ route('profile.admin.index') }}">
                    ...
                    <input type="number" name="page" id="page" min="1" max="{{ $users->lastPage() }}" value="{{ $users->currentPage() }}" class="border rounded px-2 py-1">
